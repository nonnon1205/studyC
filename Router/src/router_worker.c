...
void *router_worker(void *arg)
{
	...
	// TCPサーバーへの接続をリトライする
	GLOG_INFO("[Router] Viewer (TCP Port: %d) への接続を開始します...",
			  dest_tcp_port);
	for (int i = 0; i < CONNECT_MAX_RETRIES; i++)
	{
		DBG("TCP接続試行 #%d to 127.0.0.1:%d", i + 1, dest_tcp_port);
		if (connect(tcp_sock, (struct sockaddr *)&dest_addr,
					sizeof(dest_addr)) == 0)
		{
			tcp_connected = 1;
			break;
		...
		GLOG_WARN(
			"[Router] TCP接続試行 #%d 失敗: %s. %d秒後にリトライします...",
			i + 1, safe_strerror(errno), CONNECT_RETRY_DELAY_S);
	...
	if (!tcp_connected)
	{
		GLOG_ERR("[Router] "
				 "TCPサーバー(Viewer)"
				 "への接続に複数回失敗しました。プロセスを終了します。");
		close(tcp_sock);
		send_fatal_event(ctx->main_msqid,
						 "router_worker: TCP connect failure after retries");
		return NULL;
	}
	tcp_connected = 1;
	...
	while (1)
	{
		if (msgrcv(ctx->ipc_msqid, &notify,
				   sizeof(IpcNotifyMessage) - sizeof(long), MSG_TYPE_SHM_NOTIFY,
				   0) != -1)
		{
			...
			{
				GLOG_INFO(
					"[Router] SHM終了通知を受信しました。ループを抜けます。");
				break;
			}
			...
			{
				DBG("SHM読出し成功: status=%d, payload=\"%s\"", status_read,
					payload);
				GLOG_INFO("  -> [Router] SHM(status=%d)からデータ読出成功: %s",
						  status_read, payload);
				...
				if (tcp_connected && tcp_sock >= 0)
				{
					...
					if (send(tcp_sock, tcp_buf, strlen(tcp_buf), MSG_NOSIGNAL) >
						0)
					{
					...
					else
					{
						if (errno == EPIPE || errno == ECONNRESET)
						{
							GLOG_INFO(
								"[Router] "
								"Viewerの切断(EPIPE/"
								"ECONNRESET)を検知しました。終了します。");
							send_signal_event(ctx->main_msqid, SIGTERM);
							break;
						}
						else
						{
							GLOG_ERR("  -> [Router] TCP送信エラー(一時的): %s",
									 safe_strerror(errno));
						}
					}
				}
