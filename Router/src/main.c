...
{
	fprintf(stderr,
			"Usage: %s [--fail-leak] [--fail-use-after-free] [--fail-race]\n",
			prog);
}
...
static void send_shm_quit(int msqid)
{
	...
	if (msgsnd(msqid, &notify, sizeof(IpcNotifyMessage) - sizeof(long), 0) ==
		-1)
	{
...
int main(int argc, char *argv[])
{
	...
	if (router_ctx.ipc_msqid != -1)
	{
		...
		while (msgrcv(router_ctx.ipc_msqid, &dummy,
					  sizeof(dummy) - sizeof(long), 0,
	...
	/* 全体として SIGPIPE を無視（各関数がシグナルではなく errno=EPIPE
	 * でエラーを返すようにする） */
	...
	if (mgmt_ok)
	{
		...
		{
			GLOG_ERR("[Mgmt] mgmt_worker スレッド作成失敗: %s",
					 safe_strerror(errno));
			mgmt_ok = 0;
		}
	}
	...
	while (g_keep_running)
	{
		...
		if (msgrcv(mainmsqid, &rx_msg, sizeof(InternalMsg) - sizeof(long), 0,
				   0) == -1)
		{
			if (errno == EINTR)
				continue;
		...
		{
		case EV_QUIT:
			GLOG_INFO("[Main] 外部 UDP QUIT "
					  "を受信しました。終了命令として扱いません。");
			break;
		case EV_UDP:
			GLOG_INFO("[Main] UDPデータ受信: %s", rx_msg.data.udp_payload);
			break;
		case EV_SIGNAL:
			GLOG_INFO("[Main] シグナル(%d)を検知しました。",
					  rx_msg.data.sig_num);
			if (rx_msg.data.sig_num == SIGINT || rx_msg.data.sig_num == SIGTERM)
			{
				atomic_store_explicit(&g_keep_running, false,
...
cleanup_t2:
	if (t2_created)
	{
		...
		{
			GLOG_ERR("pthread_cancel(signal_worker): %s",
					 safe_strerror(cancel_ret));
		}
...
cleanup_main_msq:
	if (msgctl(mainmsqid, IPC_RMID, NULL) == -1)
		GLOG_ERR("msgctl(IPC_RMID): %s", safe_strerror(errno));
cleanup_log:
