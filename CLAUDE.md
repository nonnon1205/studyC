...
**Claude はコードを変更してビルドが成功したら、必ず以下を順番に実行してからタスク完了を報告すること。**
...
**push 前の動作確認はユーザーが行う。** 自動テストで検出できない不具合（特定条件での再現、マルチスレッド競合等）はユーザーが手動で確認してから push する。
...
`.clang-format` で自動整形する。手動で整形する必要はない。

```bash
clang-format -i <ファイル>   # 単一ファイル
```

| 項目 | 設定 |
|---|---|
| 括弧スタイル | Allman（開き括弧を次の行に） |
| インデント | タブ・4文字 |
| ポインタ | `int *p` スタイル（変数側に `*`） |
| 空行 | 最大1行 |
| 行幅 | 80文字 |
| インクルードソート | しない（依存順序が壊れるため） |
