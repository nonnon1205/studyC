						...
						MgmtCommandResponse *resp, void *ctx)
{
	(void)ctx;
	const char *msg = "pong";
	mgmt_response_init(resp, req->request_id, MGMT_RESULT_OK, "viewer", msg,
					   strlen(msg) + 1);
	return 0;
}
							  ...
							  MgmtCommandResponse *resp, void *ctx)
{
	...
	snprintf(buf, sizeof(buf), "{\"status\":\"running\",\"uptime_s\":%ld}",
			 uptime);
	mgmt_response_init(resp, req->request_id, MGMT_RESULT_OK, "viewer", buf,
					   strlen(buf) + 1);
	return 0;
}
...
{
	if (handler_register("viewer", MGMT_CMD_PING, handler_ping, ctx) < 0)
		return -1;
	if (handler_register("viewer", MGMT_CMD_GET_STATUS, handler_get_status,
						 ctx) < 0)
		return -1;
	if (handler_register("viewer", MGMT_CMD_SHUTDOWN, handler_shutdown, ctx) <
		0)
		return -1;
	return 0;
}
