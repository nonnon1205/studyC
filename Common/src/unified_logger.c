...
static void ulog_output(UlogHandle logger, UlogLevel level, const char *file,
...
{
	struct UlogContext *logger =
		(struct UlogContext *)malloc(sizeof(struct UlogContext));
	if (!logger)
		return NULL;
...
int ulog_stats(UlogHandle logger, uint64_t *total_logs, uint64_t *dropped_logs)
{
	...
	pthread_mutex_lock(&logger->lock);
	if (total_logs)
		*total_logs = logger->total_messages + logger->dropped_messages;
	if (dropped_logs)
		*dropped_logs = logger->dropped_messages;
	pthread_mutex_unlock(&logger->lock);
...
UlogHandle log_get_handle(void) { return g_default_logger; }
