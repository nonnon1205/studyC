...
static bool safe_lock(pthread_mutex_t *mtx) {
    ...
    if (ret == EOWNERDEAD) {
        syslog(LOG_WARNING, "[POSIX_SHM] ロック保持者の死亡を検知。状態を復旧します。");
        pthread_mutex_consistent(mtx);
        return true;
    }
#endif

    syslog(LOG_ERR, "[POSIX_SHM] Mutexロック失敗: %s", strerror(ret));
    return false;
}
...
ShmHandle shm_api_init(void) {
    ...
    // メモリマッピング
    ctx->shm_ptr = (SharedData*)mmap(NULL, sizeof(SharedData), PROT_READ | PROT_WRITE, MAP_SHARED, ctx->shm_fd, 0);
