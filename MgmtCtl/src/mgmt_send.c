					  ...
					  MgmtCommandResponse *resp, int timeout_ms)
{
	...
	snprintf(cli.sun_path, sizeof(cli.sun_path), "/tmp/studyc_mgmtctl_%d.sock",
			 (int)getpid());
	unlink(cli.sun_path);
	...
	if (sendto(fd, req, sizeof(MgmtCommandRequest), 0, (struct sockaddr *)&srv,
			   sizeof(srv)) < 0)
		goto cleanup;
