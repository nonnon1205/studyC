...
static const char *module_to_path(const char *module)
{
	if (strcmp(module, "collector") == 0)
		return MGMT_SOCKET_PATH_COLLECTOR;
	if (strcmp(module, "router") == 0)
		return MGMT_SOCKET_PATH_ROUTER;
	if (strcmp(module, "viewer") == 0)
		return MGMT_SOCKET_PATH_VIEWER;
	return NULL;
}
...
static int parse_command(const char *s, uint8_t *out)
{
	...
	{
		const char *name;
		MgmtCommandType type;
	} table[] = {{"ping", MGMT_CMD_PING},
...
int main(int argc, char *argv[])
{
	...
	mgmt_request_init(&req, cmd_type, module, payload_len ? payload : NULL,
					  payload_len);
	...
	printf("[%s] %s -> %s\n", module, mgmt_command_str(cmd_type),
