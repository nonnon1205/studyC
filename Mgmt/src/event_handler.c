...
int handler_list(char* buf, size_t buf_size)
{
    ...
    for (int i = 0; i < g_registry.entry_count; i++) {
        ...
        uint64_t avg_latency = (entry->invocation_count > 0) ?
                              entry->total_latency_us / entry->invocation_count : 0;

        offset += snprintf(buf + offset, buf_size - offset,
                          "{\"module\":\"%s\",\"command\":\"%s\",\"invocations\":%u,"
                          "\"avg_latency_us\":%lu}",
                          entry->module_name,
                          mgmt_command_str(entry->cmd_type),
...
{
    if (!total_cmds || !total_fails) return -1;
