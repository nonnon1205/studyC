...
{
    struct MetricsCollectorState* mc =
        (struct MetricsCollectorState*)malloc(sizeof(struct MetricsCollectorState));
    if (!mc) return NULL;
...
int metrics_update_collector(MetricsHandle handle, uint64_t latency_us, int success)
{
    if (!handle) return -1;
