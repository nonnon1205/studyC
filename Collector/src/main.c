...
int main(void)
{
	...
	if (ipc_msqid != -1)
	{
		...
		while (msgrcv(ipc_msqid, &dummy, sizeof(dummy) - sizeof(long), 0,
	...
	CollectorMgmtCtx mgmt_ctx = {.start_time = time(NULL),
	...
	MgmtSocketHandle mgmt_sock = mgmt_socket_create(MGMT_SOCKET_PATH_COLLECTOR);
	if (!mgmt_sock)
	{
