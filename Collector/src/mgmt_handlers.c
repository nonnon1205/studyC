...
{
    if (handler_register("collector", MGMT_CMD_PING,
                         handler_ping, ctx) < 0) return -1;
    if (handler_register("collector", MGMT_CMD_GET_STATUS,
                         handler_get_status, ctx) < 0) return -1;
    if (handler_register("collector", MGMT_CMD_SHUTDOWN,
